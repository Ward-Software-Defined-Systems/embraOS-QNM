...
The architecture is wired and inert-by-default; the experiment that decides whether ψ is load-bearing **in fact** — not just by claim — has run its first full arc. The result so far: a frozen *instruct* Core cannot carry a constitutive Embra, and the work is pivoting to a *base* Core (see "Full circle," below).
