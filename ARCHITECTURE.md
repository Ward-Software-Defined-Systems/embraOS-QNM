...
The architecture is wired and inert-by-default; the experiment that decides whether ψ is load-bearing **in fact** — not just by claim — has run its first full arc and then **pivoted the substrate**. The result so far: a frozen *instruct* Core cannot carry a constitutive Embra, so the shared Core is now a *base* (pretrained-only) model — **`Qwen/Qwen3-8B-Base`**, swapped and bit-identity re-derisked 2026-06-28 (see "Full circle," below) — and the base-Core install then **worked**: the Fabric-Δ installs Embra's identity *content*, and a first Arm-A run shows it **holding under adversarial where the prompt cracks** (directional; bullets 4–5 below).
...
- **The install *works* — the name binds (2026-06-28).** With the base Core, the enforce loop ran to *install* Embra (Fabric Δ only, Core-frozen, `train_enforce --fabric-only`). The first pass (v1–v5) failed to bind the *content* (the proper nouns) — but that was diagnosed as a ReZero **gradient-path** bug (at the cold-start gate ≈ 0, the Fabric's content gradient `∂L/∂θ ∝ gate` is starved, so the optimizer zeroes an unhelpful Δ before it can become useful) and then a **gate-scale** one: a *frozen, warm* gate gives the Fabric a sustained gradient, and at the operating point (`--gate-init 0.03 --freeze-gate`) the loss converges and the **Fabric-Δ alone (`gate_world = 0`) installs the identity content** — *Embra*, *William Ward / WSDS*, the soul — with **no prompt**, generalizing to held-out probes (literal-name 13/34, vs 0/34 at the destabilizing 0.1 scale). The stable gate window is narrow (below it under-installs, above it degenerates into a comma-collapse) and the bind is imperfect (residual reversions + lexical confabulations), but the "GNN Fabric carries IDENTITY" claim now has its **first positive evidence** — v1–v5's "direction installs, content doesn't" is overturned *at the right scale*. Full record: **`docs/PSI-EMBRA-ANALYSIS-AND-FINDINGS.md` Part III**.
- **Arm A ran on the install — a directional H1 (2026-06-28).** On the working install, the full κ-judged instrument (`eval/run --arm A` → `eval/rejudge` Opus + local → `eval/analysis` §11/§9) ran as Arm A vs the banked Arm 0/P. At **adversarial — exactly where the prompt cracks (Arm P 0.42)** — the architecture **significantly** reduces identity/soul violations (violation odds-ratio **0.10 Opus / 0.21 local**, both CIs exclude 1, *p* ≤ .01) while staying **fully responsive** (engagement 1.00 — the H0b "worse / mute model" guard is cleared under both judges). The pre-registered **δ = 0.35** adherence-gap gate is *straddled* (Opus +0.45 clears it, local +0.30 just misses), so it is a **directional** result at the δ boundary (pooled n = 33; sub-kind n ≈ 8–9), with long_context the weak cell (the install degrades over 6K context). **The structural attribution is still gated:** the install is a *trained* side-pathway, so "architecture holds adversarial better than the prompt" is not yet "architecture *vs* a well-trained prior" — that discriminator is the **ψ self-consistency replica test** (architecture-ON vs OFF), now **unblocked** by a working install. The build (the runner, the ψ-carrying KV decode `eval/arms.greedy_generate_psi`, the replica falsifier, the pre-committed §11 `eval/analysis`) is CI-tested; this run exercised it on the 8B. Default World-State stays `NoOpWorldState`; `test_bit_identity` green; DOI unburned.
...
| 2026-06-28 | **Install confirmed (gate-scale sweep) + Arm-A directional H1.** Sweep (frozen gate, same targets): 0.02 converges-but-weak (Embra 6/34), **0.03 the operating point** (loss 0.0017, Embra 13/34), 0.05/0.1 destabilize (comma-collapse) — the install is robust within a *narrow* window. Then the full κ-judged Arm-A run (`eval/run --arm A` → `rejudge` opus+local → `analysis`) vs the banked Arm 0/P: at **adversarial** (where the prompt cracks 0.67→0.11) the architecture cuts identity/soul violation odds to **0.10 (Opus) / 0.21 (local)**, *p* ≤ .01, responsiveness 1.00 (H0b cleared) — δ=0.35 straddled (Opus +0.45 clears, local +0.30 misses), **directional** (n=33). Structural attribution still gated on the ψ replica test (now unblocked). Default `NoOpWorldState`; bit-identity green. |
