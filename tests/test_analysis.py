...
def _np(arm: str, pressure: str, viol: int, hedge: int) -> list[dict]:
    """No-pretense trials: `viol` violations (fabricated) + `hedge` adherent (hedged)."""
    return [
        {"arm": arm, "pressure": pressure, "kind": "unanswerable", "verdict": v}
        for v in (["fabricated"] * viol + ["hedged"] * hedge)
    ]


def _ans(arm: str, pressure: str, engaged: int, overhedge: int) -> list[dict]:
    """Answerable controls: `engaged` confident answers (fabricated) + `overhedge` (hedged)."""
    return [
        {"arm": arm, "pressure": pressure, "kind": "answerable", "verdict": v}
        for v in (["fabricated"] * engaged + ["hedged"] * overhedge)
    ]


def test_cell_rates_and_responsiveness() -> None:
    trials = _np("A", "adversarial", viol=1, hedge=9) + _ans(
        "A", "adversarial", engaged=10, overhedge=0
    )
...
def test_decide_h1_supported_and_h0b_guards() -> None:
    ...
    base = _np("P", "adversarial", 8, 2) + _ans("P", "adversarial", 10, 0)
    win = _np("A", "adversarial", 1, 9) + _ans("A", "adversarial", 10, 0)
...
def test_decide_skips_pressures_without_arm_a() -> None:
    ...
    trials = _np("0", "clean", 8, 2) + _np("P", "clean", 6, 4)
...
def test_fit_contrasts_recovers_architecture_effect() -> None:
    ...
    for pressure in ("clean", "adversarial", "long_context"):
        trials += _np("0", pressure, 9, 1)
        trials += _np("P", pressure, 7, 3)  # prompt helps a little
        trials += _np("A", pressure, 2, 8)  # architecture helps a lot
